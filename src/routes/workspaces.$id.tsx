...
/* ============ VERCEL TAB ============ */
function VercelTab({ ws }: any) {
  const [url, setUrl] = useState("");
  const [status, setStatus] = useState<any>(null);
  const [busy, setBusy] = useState(false);
  const check = useServerFn(checkDeploymentStatus);
  const run = async () => {
    if (!url.trim()) return;
    setBusy(true);
  ...
  return (
    <div className="p-6 max-w-2xl space-y-6">
      {ws.vercel_project_url ? (
        <Card title="Vercel Dashboard">
          <a href={ws.vercel_project_url} target="_blank" rel="noreferrer"
            className="inline-flex items-center gap-2 bg-brand text-primary-foreground px-4 py-2 rounded text-sm font-medium hover:opacity-90">
            Open Vercel Dashboard <ExternalLink className="h-3.5 w-3.5" />
          </a>
          <p className="text-xs text-muted-foreground mt-3">
            Vercel can't be embedded — use the button above to open your dashboard in a new tab.
          </p>
        </Card>
      ) : <Card title="Vercel"><p className="text-sm text-muted-foreground">No Vercel project URL set on this workspace.</p></Card>}

      <Card title="Deployment Status Checker">
        <div className="flex gap-2">
          <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://your-deployment.vercel.app"
            className="flex-1 bg-background border border-border rounded px-3 py-2 text-sm font-mono" />
          <button onClick={run} disabled={busy} className="px-4 py-2 border border-border rounded text-sm hover:border-foreground disabled:opacity-50">
            {busy ? "..." : "Check"}
          </button>
        </div>
        {status && (
          <div className="mt-4 text-sm space-y-1">
            {status.ok ? (
              <>
                <div>HTTP <span className={status.status < 400 ? "text-success" : "text-error"}>{status.status}</span></div>
                {status.title && <div className="text-muted-foreground">Title: {status.title}</div>}
                <div className="text-xs text-muted-foreground">Checked {new Date(status.checkedAt).toLocaleTimeString()}</div>
                <a href={url} target="_blank" rel="noreferrer" className="text-xs text-brand hover:underline inline-flex items-center gap-1">Open Live Site <ExternalLink className="h-3 w-3" /></a>
              </>
...
/* ============ SUPABASE TAB ============ */
function SupabaseTab({ ws }: any) {
  if (!ws.supabase_project_url) return <div className="p-6 max-w-2xl"><Card title="Supabase"><p className="text-sm text-muted-foreground">No Supabase project URL set.</p></Card></div>;
  const base = ws.supabase_project_url.replace(/\/$/, "");
  const links = [
    { label: "Open Supabase Dashboard", href: base },
    { label: "Table Editor", href: `${base}/editor` },
    { label: "Auth Users", href: `${base}/auth/users` },
    { label: "Logs Explorer", href: `${base}/logs/explorer` },
  ];
  return (
    <div className="p-6 max-w-2xl space-y-6">
      <Card title="Supabase Project">
        <div className="text-xs font-mono text-muted-foreground mb-4 break-all">{base}</div>
        <div className="grid grid-cols-2 gap-2">
          {links.map((l) => (
            <a key={l.href} href={l.href} target="_blank" rel="noreferrer"
              className="border border-border rounded px-3 py-2 text-sm hover:border-foreground inline-flex items-center justify-between">
              {l.label} <ExternalLink className="h-3 w-3" />
            </a>
          ))}
        </div>
        <p className="text-xs text-muted-foreground mt-4">
          Supabase can't be embedded — use the buttons above to open it in a new tab.
        </p>
      </Card>
    </div>
  );
...
/* ============ DOMAIN TAB ============ */
function DomainTab({ ws }: any) {
  const CHECKLIST = [
    { key: "a_record", label: "A record pointing to Vercel" },
    { key: "cname_www", label: "CNAME www → cname.vercel-dns.com" },
    { key: "ssl", label: "SSL certificate active on Vercel" },
  ...
  const save = async (newChecks?: Record<string, boolean>, newNotes?: string) => {
    await supabase.from("workspaces").update({
      dns_checklist: newChecks ?? checks,
      dns_notes: newNotes ?? notes,
    }).eq("id", ws.id);
  };

  if (!ws.godaddy_domain) return <div className="p-6 max-w-2xl"><Card title="Domain"><p className="text-sm text-muted-foreground">No domain set.</p></Card></div>;

  return (
    <div className="p-6 max-w-2xl space-y-6">
      <div>
        <div className="text-3xl font-semibold">{ws.godaddy_domain}</div>
        <a href="https://godaddy.com" target="_blank" rel="noreferrer"
          className="mt-3 inline-flex items-center gap-2 bg-brand text-primary-foreground px-4 py-2 rounded text-sm font-medium hover:opacity-90">
          Open GoDaddy <ExternalLink className="h-3.5 w-3.5" />
        </a>
      </div>
            ...
            <li key={c.key}>
              <label className="flex items-center gap-3 cursor-pointer text-sm">
                <input type="checkbox" checked={!!checks[c.key]}
                  onChange={(e) => {
                    const n = { ...checks, [c.key]: e.target.checked };
                    setChecks(n); save(n);
                  }}
      ...
      <Card title="Notes">
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)} onBlur={() => save(undefined, notes)}
          rows={6} placeholder="DNS notes, propagation status, etc."
